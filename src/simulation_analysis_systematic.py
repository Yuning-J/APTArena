...
class SystematicAnalyzer:
    ...
    def generate_visualizations(self, metrics_by_strategy: Dict, output_dir: str):
        ...
        # 2. Risk Trajectory Comparison
        self.plot_risk_trajectories(metrics_by_strategy, output_path)
        
        # 3. Performance Distribution
        self.plot_performance_distributions(metrics_by_strategy, output_path)
        
        # 4. ROI vs Cost Scatter
        self.plot_roi_cost_scatter(metrics_by_strategy, output_path)
        
        # 5. Time-to-Detection Comparison
        self.plot_time_metrics(metrics_by_strategy, output_path)
    ...
    def plot_performance_distributions(self, metrics_by_strategy: Dict, output_path: Path):
        ...
        sns.boxplot(data=df, x='Strategy', y='Net Value', ax=axes[0,0])
        axes[0,0].set_title('Net Business Value Distribution')
        strategy_names = df['Strategy'].unique()
        strategy_names = [s.replace('_', '\n') for s in strategy_names]
        axes[0,0].set_xticklabels(strategy_names, rotation=0)
        ...
        sns.boxplot(data=df, x='Strategy', y='ROI', ax=axes[0,1])
        axes[0,1].set_title('Security ROI Distribution')
        axes[0,1].set_xticklabels(strategy_names, rotation=0)
        ...
        sns.boxplot(data=df, x='Strategy', y='Compromised Assets', ax=axes[1,0])
        axes[1,0].set_title('Compromised Assets Distribution')
        axes[1,0].set_xticklabels(strategy_names, rotation=0)
        ...
        sns.boxplot(data=df, x='Strategy', y='Attack Success Rate', ax=axes[1,1])
        axes[1,1].set_title('Attack Success Rate Distribution')
        axes[1,1].set_xticklabels(strategy_names, rotation=0)
    ...
    def plot_time_metrics(self, metrics_by_strategy: Dict, output_path: Path):
        """Plot time-to-detection and time-to-mitigate metrics"""
        ...
        if ttd_data:
            ...
            sns.boxplot(data=df_ttd, x='Strategy', y='Time to Detection', ax=axes[0])
            axes[0].set_title('Time to Detection Distribution')
        ...
        if ttm_data:
            ...
            sns.boxplot(data=df_ttm, x='Strategy', y='Time to Mitigate', ax=axes[1])
            axes[1].set_title('Time to Mitigate Distribution')
            axes[1].tick_params(axis='x', rotation=0)
        
        plt.tight_layout()
        plt.savefig(output_path / 'time_metrics.png', dpi=300, bbox_inches='tight')
        plt.close()
...
"""
        ...
        for stage in attack_stages:
            ...
            for trial in trials:
                ...
                stage_attempts = [attempt for attempt in exploit_attempts
                                  ...
                                  (attempt.get('action_type', '') == 'exfiltration' and stage == 'Exfiltration') or
                                  (attempt.get('action_type', '') == 'initial_access' and stage == 'Initial Access')]
    ...
    def generate_enhanced_visualizations(self, metrics_by_strategy: Dict, output_dir: str):
        """Generate enhanced visualizations with new metrics"""
        output_path = Path(output_dir)
        output_path.mkdir(exist_ok=True)
        ...
        self.plot_cybersecurity_metrics(metrics_by_strategy, output_path)
        self.plot_attack_defense_balance(metrics_by_strategy, output_path)
        self.plot_security_posture_evolution(metrics_by_strategy, output_path)
    
    def plot_cybersecurity_metrics(self, metrics_by_strategy: Dict, output_path: Path):
        """Plot comprehensive cybersecurity metrics"""
        fig, axes = plt.subplots(2, 2, figsize=(16, 12))
        axes = axes.flatten()
        
        # Plot 1: Detection and Response Metrics
        ax1 = axes[0]
        strategies = list(metrics_by_strategy.keys())
        detection_latencies = []
        response_times = []
        ...
        for strategy in strategies:
            metrics_list = metrics_by_strategy[strategy]
            avg_detection = np.mean([m.detection_latency for m in metrics_list])
            avg_response = np.mean([m.response_time for m in metrics_list])
            detection_latencies.append(avg_detection)
            response_times.append(avg_response)
        
        x = np.arange(len(strategies))
        width = 0.35
        
        ax1.bar(x - width/2, detection_latencies, width, label='Detection Latency', alpha=0.8)
        ax1.bar(x + width/2, response_times, width, label='Response Time', alpha=0.8)
        
        ax1.set_xlabel('Strategy')
        ax1.set_ylabel('Time (steps)')
        ax1.set_title('Detection and Response Performance')
        ax1.set_xticks(x)
        strategy_names = [s.replace('_', '\n') for s in strategies]
        ...
        # Plot 2: Threat Intelligence Accuracy
        ax2 = axes[1]
        ti_accuracies = []
        ...
        for strategy in strategies:
            metrics_list = metrics_by_strategy[strategy]
            avg_ti_accuracy = np.mean([m.threat_intelligence_accuracy for m in metrics_list])
            avg_fp_rate = np.mean([m.false_positive_rate for m in metrics_list])
            ti_accuracies.append(avg_ti_accuracy)
            false_positive_rates.append(avg_fp_rate)
        
        ax2.bar(strategies, ti_accuracies, alpha=0.8, label='TI Accuracy')
        ax2_twin = ax2.twinx()
        ax2_twin.plot(strategies, false_positive_rates, 'ro-', label='False Positive Rate')
        
        ax2.set_ylabel('Threat Intelligence Accuracy')
        ax2_twin.set_ylabel('False Positive Rate')
        ax2.set_title('Threat Intelligence Performance')
        ax2.tick_params(axis='x', rotation=0)
        
        # Plot 3: Security Posture Metrics
        ax3 = axes[2]
        patch_effectiveness = []
        ...
        for strategy in strategies:
            metrics_list = metrics_by_strategy[strategy]
            avg_patch_eff = np.mean([m.patch_effectiveness for m in metrics_list])
            avg_sec_improvement = np.mean([m.security_posture_improvement for m in metrics_list])
            patch_effectiveness.append(avg_patch_eff)
            security_improvements.append(avg_sec_improvement)
        
        ax3.scatter(patch_effectiveness, security_improvements, s=100, alpha=0.7)
        for i, strategy in enumerate(strategies):
            ax3.annotate(strategy, (patch_effectiveness[i], security_improvements[i]), 
                        xytext=(5, 5), textcoords='offset points')
        
        ax3.set_xlabel('Patch Effectiveness')
        ax3.set_ylabel('Security Posture Improvement')
        ax3.set_title('Security Posture Analysis')
        ax3.tick_params(axis='x', rotation=0)
        ax3.grid(True, alpha=0.3)
        
        # Plot 4: Business Continuity and Risk
        ax4 = axes[3]
        business_continuity = []
        risk_reduction = []
        ...
        for strategy in strategies:
            metrics_list = metrics_by_strategy[strategy]
            avg_bc = np.mean([m.business_continuity_score for m in metrics_list])
            avg_risk_red = np.mean([m.risk_reduction_ratio for m in metrics_list])
            business_continuity.append(avg_bc)
            risk_reduction.append(avg_risk_red)
        
        ax4.barh(strategies, business_continuity, alpha=0.8, label='Business Continuity')
        ax4_twin = ax4.twinx()
        ax4_twin.barh(strategies, risk_reduction, alpha=0.5, label='Risk Reduction')
        
        ax4.set_xlabel('Business Continuity Score')
        ax4_twin.set_xlabel('Risk Reduction Ratio')
        ax4.set_title('Business Continuity vs Risk Reduction')
        
        plt.tight_layout()
        plt.savefig(output_path / 'cybersecurity_metrics.png', dpi=300, bbox_inches='tight')
        plt.close()

    def plot_attack_defense_balance(self, metrics_by_strategy: Dict, output_path: Path):
        """Plot attack-defense balance analysis"""
        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 6))
        
        strategies = list(metrics_by_strategy.keys())
        
        # Plot 1: Attack Success vs Defense Effectiveness
        attack_success_rates = []
        defense_effectiveness = []
        
        for strategy in strategies:
            metrics_list = metrics_by_strategy[strategy]
            avg_attack_success = np.mean([m.attack_success_rate for m in metrics_list])
            avg_defense_eff = np.mean([m.patch_effectiveness for m in metrics_list])
            attack_success_rates.append(avg_attack_success)
            defense_effectiveness.append(avg_defense_eff)
        
        scatter = ax1.scatter(attack_success_rates, defense_effectiveness, s=100, alpha=0.7)
        for i, strategy in enumerate(strategies):
            ax1.annotate(strategy, (attack_success_rates[i], defense_effectiveness[i]), 
                        xytext=(5, 5), textcoords='offset points')
        
        ax1.set_xlabel('Attack Success Rate (%)')
        ax1.set_ylabel('Defense Effectiveness')
        ax1.set_title('Attack-Defense Balance')
        ax1.tick_params(axis='x', rotation=0)
        ax1.grid(True, alpha=0.3)
        
        # Plot 2: Security Posture vs Business Continuity
        security_postures = []
        business_continuity = []
        
        for strategy in strategies:
            metrics_list = metrics_by_strategy[strategy]
            avg_security = np.mean([m.security_posture_improvement for m in metrics_list])
            avg_business = np.mean([m.business_continuity_score for m in metrics_list])
            security_postures.append(avg_security)
            business_continuity.append(avg_business)
        
        ax2.scatter(security_postures, business_continuity, s=100, alpha=0.7)
        for i, strategy in enumerate(strategies):
            ax2.annotate(strategy, (security_postures[i], business_continuity[i]), 
                        xytext=(5, 5), textcoords='offset points')
        
        ax2.set_xlabel('Security Posture Improvement')
        ax2.set_ylabel('Business Continuity Score')
        ax2.set_title('Security vs Business Continuity')
        ax2.tick_params(axis='x', rotation=0)
        ax2.grid(True, alpha=0.3)
        
        plt.tight_layout()
        plt.savefig(output_path / 'attack_defense_balance.png', dpi=300, bbox_inches='tight')
        plt.close()

    def plot_security_posture_evolution(self, metrics_by_strategy: Dict, output_path: Path):
        """Plot security posture evolution over time"""
        ...
        for i, strategy in enumerate(strategies):
            ...
            # Calculate security posture evolution (simplified model)
            steps = list(range(1, 26))
            posture_evolution = []
            
            for step in steps:
                # Simulate security posture improvement over time
                ...
                step_posture = base_posture * (1 + improvement_rate * step / 50)
                posture_evolution.append(step_posture)
            
            ax.plot(steps, posture_evolution, label=strategy, color=colors[i], 
