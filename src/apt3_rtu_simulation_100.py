...
from src.canonical_kpis import compute_canonical_kpis
...
# Tactic values that mean "not identified" — only record detection when tactic is not in this set
_UNIDENTIFIED_TACTIC = frozenset({None, '', 'Unknown', 'None'})
...
class APT3RTUSimulation:
    ...
    def __init__(self, data_file: str, num_steps: int, defender_budget: int, attacker_budget: int,
        ...
        self._tactic_first_use_step = {}  # tactic -> step when first used (for MTTD)
        self.attack_disrupted = False
        self.current_step = 0
        self._lateral_movement_attempts = {}
        # Canonical KPIs: incident-level log for MTTD/MTTR/PAR (same definitions as legacy table)
        self._incident_rows = []
    ...
    def run_step(self, strategy, step, verbose=False):
        ...
        for action in attacker_actions:
            ...
            if is_recon:
                ...
                if action_result.get('action_result', False) and action.get('action_type') in ['initial_access',
                    ...
                    cve_id = target_vuln if target_vuln else (self.vuln_lookup.get(vuln_key, (None, None, None))[0].cve_id if vuln_key and vuln_key in self.vuln_lookup else None)
                    if vuln_key:
                        exploit_event = ExploitEvent(
                            vuln_id=target_vuln or (cve_id or ''),
                            step=step,
                            asset_id=action_result.get('target_asset'),
                            success=True,
                    ...
                    # Canonical incident: one per successful exploit (event count; do not deduplicate by asset)
                    incident_id = f"{self._current_run_id}-{len(self._incident_rows):03d}"
                    self._incident_rows.append({
                        'run_id': self._current_run_id,
                        'incident_id': incident_id,
                        'asset_id': str(action_result.get('target_asset', '')),
                        'stage': action_result.get('action_type', action.get('action_type', '')),
                    ...
                    if action.get('tactic'):
                        ...
                        self._tactic_first_use_step.setdefault(action['tactic'], step)

        # DEFENDER PHASE: Apply strategy-specific learning and patching
        # MTTD = (step attack is detected) - (step exploit is triggered by the attacker).
        # Record "exploit triggered" step when we first see this tactic (any observation).
        # Observation -> detection is stochastic; if we detect, time_to_detection = step_detected - step_triggered.
        for observation in attack_observations:
            tactic = observation.get('tactic')
            if tactic and tactic not in _UNIDENTIFIED_TACTIC:
                self._tactic_first_use_step.setdefault(tactic, step)  # step exploit was triggered (first time we see this tactic)
                if tactic not in self.mitre_techniques_detected and random.random() < observation.get('detection_confidence', 0.9):
                    self.mitre_techniques_detected.add(tactic)
                    step_triggered = self._tactic_first_use_step[tactic]
                    self.time_to_detection[tactic] = max(0, step - step_triggered)  # MTTD = step detected - step exploit triggered
                    # Canonical: assign t_detect to all incidents with this tactic not yet detected
                    for inc in self._incident_rows:
                        if inc.get('tactic') == tactic and inc.get('t_detect') is None:
                            inc['t_detect'] = step
                            inc['detected'] = True
        ...
        # Canonical: recovery = step when the vuln that was exploited is patched
        patched_cves_this_step = {v.cve_id for v in applied_patches}
        for inc in self._incident_rows:
            if inc.get('t_recover') is None and inc.get('cve_id') in patched_cves_this_step:
                inc['t_recover'] = step
                inc['recovered'] = True
    ...
    def calculate_metrics(self, state, total_patch_cost, total_patches, verbose=False):
        ...
        if verbose:
            ...
            print(f"Avg Time to Detection (steps): {avg_time_to_detection:.1f}")
    ...
    def compare_strategies(self, defender_budget: int, num_steps: int, num_trials: int = 100,
                           verbose: bool = False, hybrid_ti_weight: float = 0.4, hybrid_rl_weight: float = 0.6):
        print(
            f"\n{'=' * 80}\nCOMPARING STRATEGIES - Budget: ${defender_budget:,.2f}, Steps: {num_steps}, Trials: {num_trials}\n{'=' * 80}")
        self.defender_budget = defender_budget
        self.num_steps = num_steps
        if abs((hybrid_ti_weight + hybrid_rl_weight) - 1.0) > 1e-6:
            raise ValueError("hybrid_ti_weight and hybrid_rl_weight must sum to 1.0")
        ...
        strategies = {
            ...
            'Hybrid Defender': HybridStrategy(
                threat_intelligence_weight=hybrid_ti_weight,
                rl_weight=hybrid_rl_weight
            ),
        }
        def _load_best_q_table(base_dir):
            # Prefer non-empty Q-tables with the most learned entries.
            if not os.path.exists(base_dir):
                return None, None, "RL training results directory not found"
            training_dirs = [d for d in os.listdir(base_dir) if d.startswith('rl_defender_training_')]
            if not training_dirs:
                return None, None, "No RL training directories found"

            best_q = None
            best_path = None
            best_entries = -1
            for training_dir in sorted(training_dirs, reverse=True):
                q_table_path = os.path.join(base_dir, training_dir, "q_table.pkl")
                ...
                try:
                    ...
                    if entries > best_entries:
                        ...
                        best_q = q_table
                        best_path = q_table_path
                except Exception:
                    continue

            if best_q is None:
                return None, None, "No readable Q-table found"
            return best_q, best_path, None
        ...
        aggregated_results = {
            strategy_name: {
                ...
                'run_statistics': {
                    ...
                    'incidents_total_runs': [],
                    'canonical_mttd_steps_runs': [],
                    'canonical_mttr_steps_runs': [],
                    'protected_asset_ratio_pct_runs': []
                }
        ...
        for trial in range(num_trials):
            ...
            for strategy_name, strategy_obj in strategies.items():
                ...
                self.mitre_techniques_detected = set()
                self.time_to_detection = {}
                self._tactic_first_use_step = {}
                self.attack_disrupted = False
                self._incident_rows = []
                self._current_run_id = trial
                ...
                if strategy_name == 'RL Defender':
                    ...
                    q_table, q_table_path, load_error = _load_best_q_table(rl_results_dir)
                ...
                # Canonical KPIs (same definitions/scales as legacy table): from incident-level timestamps
                assets_total = len(self.state.system.assets)
                canonical_summary = compute_canonical_kpis(self._incident_rows, assets_total)
                # JSON-serializable incident rows (nan -> None)
                def _nan_to_none(v):
                    if isinstance(v, float) and np.isnan(v):
                        return None
                    return v
                canonical_incident_rows = [
                    {k: _nan_to_none(v) for k, v in inc.items()}
                    for inc in self._incident_rows
                ]
                canonical_run_summary = {k: _nan_to_none(v) for k, v in canonical_summary.items()}
                ...
                results.update({
                    ...
                    'canonical_incident_rows': canonical_incident_rows,
                ...
                aggregated_results[strategy_name]['run_statistics']['incidents_total_runs'].append(
                    canonical_summary['incidents_total'])
                mttd_val = canonical_summary['mttd_steps']
                mttr_val = canonical_summary['mttr_steps']
                aggregated_results[strategy_name]['run_statistics']['canonical_mttd_steps_runs'].append(
                    mttd_val if not (isinstance(mttd_val, float) and np.isnan(mttd_val)) else None)
                aggregated_results[strategy_name]['run_statistics']['canonical_mttr_steps_runs'].append(
                    mttr_val if not (isinstance(mttr_val, float) and np.isnan(mttr_val)) else None)
                aggregated_results[strategy_name]['run_statistics']['protected_asset_ratio_pct_runs'].append(
                    canonical_summary['protected_asset_ratio_pct'])
                ...
                # Canonical KPI logs (same definitions/scales as legacy table; for reviewer-proof stats)
                canonical_log = os.path.join(strategy_dirs[strategy_name], f"canonical_trial_{trial + 1}.json")
                with open(canonical_log, 'w') as f:
                    json.dump({
                        "run_id": trial,
                        "incidents": results['canonical_incident_rows'],
                        "run_summary": results['canonical_run_summary']
        ...
        for strategy_name in strategies:
            metrics = [
                ...
                'hybrid_performance',
                'incidents_total', 'canonical_mttd_steps', 'canonical_mttr_steps', 'protected_asset_ratio_pct'
            ]
            for metric in metrics:
                key = metric + '_runs'
                if key not in aggregated_results[strategy_name]['run_statistics']:
                    continue
                ...
                elif metric in ('canonical_mttd_steps', 'canonical_mttr_steps'):
                    aggregated_results[strategy_name]['mean_metrics'][metric] = np.mean(
                        valid_values) if valid_values else None
                    aggregated_results[strategy_name]['std_metrics'][metric] = np.std(
                        valid_values) if valid_values else 0.0
...
def build_parser() -> argparse.ArgumentParser:
    """Build the CLI parser for the APT3 simulation runner."""
    ...
    parser.add_argument("--hybrid-ti-weight", type=float, default=0.4, help="Hybrid TI weight (must sum to 1 with RL weight)")
    parser.add_argument("--hybrid-rl-weight", type=float, default=0.6, help="Hybrid RL weight (must sum to 1 with TI weight)")
    return parser


def run_cli(args: argparse.Namespace) -> None:
    """Execute simulation using parsed CLI arguments."""
    ...
    try:
        ...
        results = simulation.compare_strategies(
            ...
            hybrid_ti_weight=args.hybrid_ti_weight,
